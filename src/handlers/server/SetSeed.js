...
module.exports = function registerSetSeedHandler(connection) {
    connection.on(`server:${MESSAGE_IDS.SET_SEED}`, (message) => {
        ...
        if (message.U === localPlayerID) {
            inventory.removeItem(message.BlockType, INVENTORY_ITEM_TYPES.SEED, 1);
        }
    });
};